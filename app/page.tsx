...
function Hero() {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <Image
        src="https://naturfreunde-wilhelmsburg.at/wp-content/uploads/weptile-image-slider-cache/Header-2020-Berg-resized-1200x240.jpg"
        alt="Berglandschaft Wilhelmsburg"
        fill
        className="object-cover object-center scale-110"
        priority
      />
      <div className="absolute inset-0 bg-gradient-to-br from-green-900/90 via-green-800/80 to-green-700/70" />
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center text-white py-32">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 animate-fadeInUp delay-300">
          <Link href="/mitglied-werden"
            className="inline-flex items-center gap-2 px-6 py-3.5 bg-white text-green-800 font-bold rounded-xl hover:bg-green-50 transition-all shadow-lg hover:-translate-y-0.5">
    ...
    <section className="py-12 bg-green-700">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {upcoming.map((event) => (
            <Link key={event.id} href={`/veranstaltungen#${event.id}`}
              ...
              <article key={item.id}
                ...
                <div className="relative h-40 overflow-hidden">
                  {item.image ? (
                    ...
                    <div className={`h-full bg-gradient-to-br ${matchedSection?.color ?? "from-green-500 to-green-700"} flex items-center justify-center text-4xl opacity-30`}>
                      {matchedSection?.icon ?? "🌿"}
                    </div>
                  )}
...
function MemberCTA() {
  const perks = [
    "Alle Sektionen & Touren", "Ermäßigte Hüttennächtigungen",
    "Vereinsbus zu Touren", "Versicherungsschutz",
    "Mitgliederportal & Galerie", "Ab nur 12 €/Jahr für Kinder",
  ];
  return (
    <section className="py-20 bg-gradient-to-br from-green-800 to-green-900 relative overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
        ...
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold mb-4">Werde Teil der Gemeinschaft</h2>
        <p className="text-green-200 text-lg mb-8 max-w-2xl mx-auto">
          Von der ersten Wanderung bis zur Hochalpintour – bei uns findest du Gleichgesinnte und unvergessliche Erlebnisse.
        </p>
